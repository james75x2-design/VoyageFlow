 ...
 * Primary AI Engine: Google Gemini API (gemini-1.5-pro)
 * Fallback AI Engine: Groq API (llama-3.3-70b-versatile)
 */
...
// Current date context relative to Wednesday, June 24, 2026
...
# Rules & Constraints
1. **Dynamic Year Context:** The current date is Wednesday, June 24, 2026 (Year: 2026). If the user states a date without a year (e.g., "August 1st"), evaluate it against the current date:
   - If the date is still in the future for this year (2026), use 2026.
   - If the date has already passed for this year (before June 24, 2026), assume they mean the upcoming occurrence in the next year (2027).
...
async function handleRequest(request) {
  ...
  try {
    ...
    try {
      rawContent = await callGemini(messages);
    } catch (geminiError) {
      ...
      try {
        rawContent = await callGroq(messages);
